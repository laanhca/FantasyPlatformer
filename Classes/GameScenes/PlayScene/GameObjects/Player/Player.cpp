...
#include "GameScenes/PlayScene/GameObjects/GameObjectStates/IdleState.h"
...
bool Player::init() {
    ...
    auto physicBodyPlayer = PhysicsBody::createBox(Size(27, 10), PhysicsMaterial(1.0f, 0.0f, 2.0f),
                                                   Vec2(0, -10));
...
bool Player::onContactPhysicBegin(PhysicsContact &contact) {
    ...
    if ((tagA == GameConst::TAG_PLAYER && tagB == GameConst::TAG_FLOOR) ||
        (tagA == GameConst::TAG_FLOOR && tagB == GameConst::TAG_PLAYER)) {
        mTouchFloor = true;
        return true;
    }
    if ((tagA == GameConst::TAG_PLAYER && tagB == GameConst::TAG_MONSTER) ||
        (tagA == GameConst::TAG_MONSTER && tagB == GameConst::TAG_PLAYER)) {
//

        return true;
    }
    if ((tagA == GameConst::TAG_PLAYER && tagB == GameConst::TAG_TRAP) ||
        (tagA == GameConst::TAG_TRAP && tagB == GameConst::TAG_PLAYER)) {
        GameConst::GameData::isOver = true;
        isDead = true;

        return true;
    }
    if ((tagA == GameConst::TAG_PLAYER && tagB == GameConst::TAG_ITEM) ||
        (tagA == GameConst::TAG_ITEM && tagB == GameConst::TAG_PLAYER)) {
